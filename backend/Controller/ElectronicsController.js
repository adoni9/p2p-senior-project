...
//Sign Up
const ElectronicsCreate = async (req, res) => {
  ...
  getCityFromCoordinates(latitude, longitude).then(async (city) => {
    try {
      ...
      if (Status == "New") {
        try {
          const Electronicss = await Electronics.create({
            userId,
            userEmail,
            brand,
            For,
...
const GetOneElectronics = async (req, res) => {
  const { id } = req.params;
  const result = await Electronics.findById(id);
...
//   const { id } = req.params;
//   try {
//     const product = await Electronics.findById(id).populate("ratings.userId");
